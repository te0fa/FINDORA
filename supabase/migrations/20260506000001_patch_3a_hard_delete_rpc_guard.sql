-- Batch 3A Security Patch: Harden fn_hard_delete_request_with_backup
-- Created: 2026-05-06
-- Description: Live-schema-safe patch for hard deletion with strict state guards.

-- 1. Ensure columns exist (Idempotent)
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name='request_delete_backups' AND column_name='deleted_at') THEN
        ALTER TABLE public.request_delete_backups ADD COLUMN deleted_at timestamptz;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name='request_delete_backups' AND column_name='deleted_by_staff_id') THEN
        ALTER TABLE public.request_delete_backups ADD COLUMN deleted_by_staff_id uuid;
    END IF;
END $$;
...
BEGIN
    ...
    -- Allowed ONLY if: archived, closed, released, or rejected
    IF NOT (
