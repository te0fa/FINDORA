...
async function main() {
  ...
  // Read migration SQL
  const migrationSql = fs.readFileSync('supabase/migrations/20260604300003_economy_stabilizer.sql', 'utf8')
  ...
  try {
    ...
    console.warn("Migration warning/error:", err.message)
    console.log("Attempting to run only the functions to ensure they are created...")
    
    // Fallback: extract and run only the main functions
    const func1 = `
    ...
    console.log("Successfully created DB functions public.fn_run_economy_stabilizer() and public.fn_get_stabilizer_multiplier()!")
  }
