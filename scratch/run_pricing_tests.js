...
const { resolveCustomerServiceFee, resolveVendorTransactionFee } = require('../src/lib/pricing/feeResolvers');
...
async function runTests() {
  ...
  async function setCustomerPhase(phaseName, updates = {}) {
  ...
  async function setVendorPhase(phaseName, updates = {}) {
  ...
  try {
    ...
    const fees = [raceRes1.fee, raceRes2.fee].sort();
    ...
    // ----------------------------------------------------
    // Pass null/invalid customerId to trigger error or simulation
    const res7 = await resolveCustomerServiceFee(null);
    results.push({
      id: 7,
